...
OUT_DIR  = os.path.join(BASE_DIR, "out", "exploratory", "r2_ceiling")
...
def main():
    ...
    # ── miss_distance ────────────────────────────────────────────────────────────────────
    has_bs = 'bat_speed' in df.columns
    md_preds = (['bat_speed'] if has_bs else []) + \
               ['attack_angle', 'intercept_y', 'intercept_x', 'swing_path_int',
                'release_speed_c', 'plate_x', 'pfx_x_adj', 'vmov_diff', 'velo_diff','pfx_z']
    df_md = df.dropna(subset=['miss_distance'] + md_preds)
    
    md_widths = {'bat_speed': 2.0, 'attack_angle': 3.0, 'intercept_y': 3.0,
                 'intercept_x': 3.0, 'swing_path_int': 3.0, 'release_speed_c': 0.3,
                 'plate_x': 0.15, 'pfx_x_adj': 2.0, 'vmov_diff': 2.0, 'velo_diff': 2.0, 'pfx_z': 2.0}
    
    n_md = sum(1 for k in range(3, len(md_preds)+1) for _ in itertools.combinations(md_preds, k))
    print(f'\nmiss_distance: {n_md} combinations, {len(df_md):,} obs')
    
    md_recs = []
    for size in range(3, len(md_preds) + 1):
        for i, combo in enumerate(itertools.combinations(md_preds, size)):
            if i % 20 == 0: print(f'  n={size}: {i}', end='\r')
            r2_bin, n_bins, n_obs = ceiling_binning(df_md, 'miss_distance', list(combo), md_widths, 10)
            r2_tree, n_leaves = ceiling_tree(df_md, 'miss_distance', list(combo), 100, 10)
            md_recs.append({'n_preds': size, 'predictors': ', '.join(combo),
                            'r2_bin': r2_bin, 'r2_tree': r2_tree,
                            'n_bins': n_bins, 'n_leaves': n_leaves})
    
    md_df = pd.DataFrame(md_recs).sort_values('r2_tree', ascending=False)
    md_df.to_csv(os.path.join(OUT_DIR, 'md_ceiling.csv'), index=False)
    print(f'\n→ md_ceiling.csv: {(md_df["r2_bin"]>0).sum()} pos bin, {(md_df["r2_tree"]>0).sum()} pos tree')
    ...
    print('\nMD:')
    print(md_df[['n_preds', 'r2_tree', 'r2_bin', 'n_leaves', 'predictors']].head(10).to_string(index=False))
    ...
    for row, (recs, title, obs) in enumerate([(md_df, 'Miss distance', 0.20)]):
