...
def run_la_model(df):
    print('\n' + '='*70)
    print('MISS DISTANCE MODEL')
    print('='*70)

    needed = ['miss_distance', 'plate_x', 'plate_z', 'release_speed_c',
              'pfx_x', 'pfx_z', 'batter', 'pitch_type', 'description']
    sub = (df[df['description'].isin(MISS)]
           .dropna(subset=needed).copy())
    print(f'n={len(sub):,}')

    # lmer: random intercept for batter + correlated random intercept
    # for batter:pitch_type (nested), fixed effects for pitch location/movement
    formula = ('miss_distance ~ release_speed_c + plate_x + plate_z + '
               'pfx_x + pfx_z + '
    ...
    shapley = compute_shapley_r2(sub, 'miss_distance',
                                  ['release_speed_c', 'plate_x', 'plate_z',
                                   'pfx_x', 'pfx_z'])
    print(f'\nShapley R²:\n{shapley.to_string()}')

    summary_plots(r, sub, 'miss_distance', label)

    # Miss distance score: lower residual miss = better contact quality
    # Negative mean residual → beating expectation (missing less than predicted)
    scores = (sub.assign(resid=sub['miss_distance'] - r['fitted'])
                 .groupby('batter')['resid']
                 .agg(mean='mean', std='std')
                 .assign(miss_dist_score=lambda x: -x['mean'])  # higher = better
                 .reset_index())
    scores.to_csv(os.path.join(OUT_DIR, 'miss_dist_scores.csv'), index=False)
...
def compute_all_scores(timing_res, tilt_res, la_res, ev_res, df):
    ...
    miss_scores     = pd.read_csv(os.path.join(OUT_DIR, 'miss_dist_scores.csv'))

    for s in [timing_scores, tilt_scores, miss_scores]:
        s['batter'] = s['batter'].astype(str)

    # Contact score: EV above expected, penalised by miss distance above expected
    ...
    ev_scores = (pd.DataFrame({'batter': ev_df['batter'], 'z_ev': ev_z})
                   .groupby('batter')['z_ev'].mean())

    contact = ev_scores.reset_index().rename(columns={'z_ev': 'contact_score'})

    name_map = (df[['batter', 'batter_name']]
    ...
    all_scores = (
        ...
        .merge(miss_scores[['batter', 'miss_dist_score']],     on='batter', how='outer')
